...
## Предложения по улучшению (от Hermes, 2026-07-24)

1. **Дубли kanban-скилла.** Есть 3 копии: `Skills/obsidian-kanban-agent-ops`, `Skills/obsidian-kanban-agent-ops 1` (шаблоны различаются!) и `Скиллы/obsidian-kanban-agent-ops`. Стоит выбрать актуальную и удалить остальные.
2. **`meeting-digest` дублирует `meeting-protocol`.** Digest сохраняет анализ в 6 отдельных файлов, protocol — всё в одну заметку (текущий рабочий формат). Предлагаю удалить digest или пометить как устаревший.
3. **Дубль папок агентов.** `Агенты/` — подмножество `Agents/`. Оставить только `Agents/`.
4. **Секреты в SKILL.md.** В `meeting-minutes` захардкожены pageId Confluence и URL — это ок, но токены правильно держать только в secrets (сейчас так и есть — 👍).
5. **AGENTS.md, раздел 6 (Git):** написано «пока нет git-репозитория» — уже неактуально, vault теперь в GitHub. Обновить.
6. **`.DS_Store` в репозитории.** Добавить в `.gitignore` (`.DS_Store`, `~$*.pptx`).
7. **Большие бинарники.** `Стратегия Dev Platform-final_7.pptx` (51 МБ) превышает лимит GitHub; при активной работе с презентациями стоит подумать про Git LFS или исключить `Работа/**/Презентации/` из git (Dropbox их и так синхронизирует).
