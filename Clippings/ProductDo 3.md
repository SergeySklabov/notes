---
title: "ProductDo"
source: "https://app.productdo.it/module/tech_pm/lesson/techpm_blueprint"
author:
published:
created: 2026-07-09
description:
tags:
  - "clippings"
---
![](https://app.productdo.it/media/lessons/03_-_%D0%A3%D1%87%D0%B8%D0%BC%D1%81%D1%8F_%D0%BE%D0%BF%D1%80%D0%B5%D0%B4%D0%B5%D0%BB%D1%8F%D1%82%D1%8C_%D0%BD%D1%83%D0%B6%D0%BD%D1%8B%D0%B5_%D0%B4%D0%B0%D0%BD%D0%BD%D1%8B%D0%B5_hDBDUQW.png)

## Service Blueprint для стартапа такси
...
Чтобы понять, как подступиться к построению стартапа Такси, мы начнем с важного инструмента: Service Blueprint. Он позволяет связать действия пользователя (шаги CJM) с фронтендом, бэкэндом и вспомогательными процессами.  
...
Итак, мы делаем iOS/Android приложение, которое позволяет вызвать такси в пару кликов, что, в переводе на язык продуктовых требований и customer journey map выражается в следующих фичах для пассажира:

- Простая регистрация в приложении
- Поиск нужного адреса и понимание длины поездки
- Выбор определенного тарифа и поиск свободного водителя по этому тарифу
- Оплата: мы договорились, что будем сразу снимать полную сумму поездки
- Трекинг положения машины и времени до места назначения
- Возможность оставить отзыв
- При необходимости возможность связаться с телефонным саппортом

Наш дизайнер собрал это все в красивом [Figma-прототипе](https://www.figma.com/proto/kDTFwrWvwNny2tedvZILWb/ProductDo---Apps?type=design&node-id=213-3301&scaling=min-zoom&page-id=0%3A1&starting-point-node-id=213%3A3301) - обязательно все прокликай, чтобы почувствовать продукт (если вдруг ссылка не работает, попробуй открыть ее в инкогнито режиме).
...
Именно, наш продукт - маркетплейс, то есть мы не просто посылаем пассажира в один из таксопарков, а даем водителям удобное приложение для таксования! Более конкретно, ожидаем следующий функционал для водителей:

- Возможность зарегистрироваться, загрузить необходимые документы
- Начать принимать и возить пассажиров
- Получать деньги за свою работу
- Видеть отзывы
- При необходимости связаться с телефонной поддержкой
...
Напомню, мы будем строить вот такую табличку, только не для магазина велосипедов, а для сервиса такси.

![](https://app.productdo.it/media/tasks/ru/tech_blue_bike_reminder_sbp_bike.png) ![](https://app.productdo.it/media/persona/Jane_from_ProductDo_zU6M7XD.jpg)

Jane  
Principal Product Manager

Начнем с CJM **пассажира**, водителя оставим на потом - там немного другой пользовательский сценарий.

Я создала [Service Blueprint-шаблон в Миро](https://miro.com/app/board/uXjVKachqeQ=/?moveToWidget=3458764584147144880&cot=14) и уже его частично заполнила для действий пользователя и фронтенд-элементов. А вот для бэка мне будет нужна твоя помощь. Скопируй себе шаблон, чтобы была возможность редактировать. Пока смотри только на первую табличку - на другие артефакты не обращай внимания. Кстати, если не нравится Миро, используй любую другую табличку, тут главное - контент.

Начнем **только** с первого шага - регистрации. Посмотри на **первый** столбец. Какие обязательные действия должны, по-твоему, произойти на бэкэнде, чтобы пользователь смог успешно зарегистрироваться и стать пассажиром нашего сервиса такси? Добавь свой ответ в табличку и выбери все логичные варианты.

![](https://app.productdo.it/media/tasks/en/techpm_blue_step_register_techpm_taxiapp_1_signup.png)

Действительно, для регистрации нам нужно будет принять и сохранить данные формы: имейл, телефон и, возможно, имя. Далее надо будет их верифицировать, чтобы удостовериться, что пользователь действительно владеет данными контактами. Для этого можно использовать простые имейл/СМС со ссылкой-подтверждением.

Наконец, надо будет зафиксировать успешную верификацию в "памяти" сервиса (о базах данных поговорим чуть позже). Ты получаешь бонусный балл, если решил напомнить забывчивому пользователю о ссылке-подтверждении еще раз через несколько дней.

![](https://app.productdo.it/media/tasks/ru/correct_solution_techpm_blue_step_register_Screenshot_2024-04-01_at_15.52.16.png)
...
Едем дальше! Точнее пока не едем, а вводим адрес. Да-да, это отдельный шаг CJM. Что должно произойти на бэкэнде, чтобы пользователь успешно ввел финальную точку? Подсказка: пассажир ожидает, что мы не заставим его вводить все буквы длинной улицы “Маршала Василевского, 133д”!

Добавь свой ответ в табличку и выбери правильные варианты:

![](https://app.productdo.it/media/tasks/en/techpm_blue_step_auto_techpm_taxiapp_2_address.png)

Не ожидал? Пользователь только ввел куда ехать, а на бэкэнде уже произошла целая эпопея: с каждым новым введенным символом сервис поиска пытался найти нужный адрес, потом нужный дом и приставку, пока, наконец, пользователь его не подтвердил. Теперь мы готовы поставить точку на карте и рассчитать маршрут из А в Б.

А теперь представь, сколько программистов трудятся над этой фичей (называемой *autocomplete*) в Google, а сколько запросов в секунду сыпятся на эти сервисы: пользователи делают примерно 100 000 поисков в секунду (!), Autocomplete вызывается, скажем, 10 раз за ввод, так что получается 1 миллион запросов в секунду. А у нашего стартапа пока это около 100, хе-хе. Ну это я отвлеклась!

![](https://app.productdo.it/media/tasks/ru/correct_solution_techpm_blue_step_auto_Screenshot_2024-04-01_at_15.53.06.png)
...
Следующие два шага (3 и 4) сделай сам: подумай, что должно произойти на бэке для шага выбора тарифа поездки и оплаты (мы берем деньги сразу и только в случае успеха ищем водителя). Сюда прикрепи скриншот, и потом уже обсудим.

![](https://app.productdo.it/media/tasks/en/techpm_blue_step_type_pay_techpm_taxiapp_3_tariff-payment-1_2.png)
...
Выбери варианты, которые есть у тебя, и у Джейн.

У меня получилось что-то такое. Не волнуйся, если пока не можешь "заметить" все действия на бэкэнде, это придет с опытом и другими кейсами. Пока просто старайся напоминать себе, что почти ни одно событие на экране не происходит без помощи сервисов. Также не переживай, если детали немного различаются - главное, что ты уловил суть.

Обрати внимание, что оплата - триггер к началу поиска водителя, связывающий шаг 4 и следующий шаг 5. Другими словами, именно оплата (а не действия пользователя) запускают следующий шаг CJM.

![](https://app.productdo.it/media/tasks/ru/correct_solution_techpm_blue_step_type_pay_compare_Screenshot_2024-06-12_at_09.12.07.png)
...
Давай разберемся с важнейшим шагом (номер 5) - поиском машины. Попробуй представить, что должно произойти на бэкэнде, чтобы пассажир узнал, кто его повезет? С непривычки это сложно, но я в тебя верю.

**Подсказка 1**  
*Нам нужно опросить водителей, пока кто-то из них не согласится везти пассажира. С какого водителя начать опрос, с любого из тысяч в большом городе?* **Подсказка 2**  
*Как узнать, что водитель готов принять заказ?* **Подсказка 3**  
*Как водитель узнает, куда ехать?*

На данном шаге происходит множество критических событий. Это, кстати, самое сердце логики всех такси-сервисов!
- Сначала мы ищем наиболее вероятных водителей: тех, кто поближе, с нормальными отзывами, с хорошим процентом неотказов - алгоритм выбора может быть довольно сложным и это отдельная бизнес-задача.
- Потом нам надо опросить водителей одного за другим. Кто-то скажет нет, а кто-то - согласится.
- Когда водитель выбран, надо отправить ему точку, где подбирать пассажира.
- Бонусный шаг: записать водителя в главный объект поездки, чтобы у нас были все факты: кто, когда, кого, за сколько вез. Запомни этот паттерн: все важные факты записываются в базу данных, чтобы считать аналитику, делать выплаты, распутывать обращения в саппорт, делать отчетность и так далее. В данном случае базу поездок, а в случае, например, с Amazon - в базу продаж.
...
Я вижу, что ты уже неплохо справляешься! Доделаешь табличку до конца (шаги с 6 по 9)? Обрати внимание, что теперь на тебе и нижняя строчка - **"Supporting processes"**. В ней попробуй указать один-два примера того, что может пойти не так, и что по этому поводу делать. Например, это может быть злой клиент, звонящий в саппорт, а может быть какая-то необходимая ручная работа, например, обновление карты города и базы данных адресов.

Сюда присылай скриншот.

![](https://app.productdo.it/media/tasks/en/techpm_blue_second_half_techpm_taxiapp_4_ride-1_1.png)
...
Вот мои шаги 5 (добавила раньше) и 6-9 (новые). Как оцениваешь свой результат по сравнению с моим?

![](https://app.productdo.it/media/tasks/ru/techpm_blue_second_half_compare_Screenshot_2025-06-18_at_08.47.41.png)
...
Поздравляю с твоей первой Service Blueprint! Могу поспорить, что в начале задания ты думал "пфф, просто вызов такси", а к концу прочувствовал глубину, с которой продакту нужно понимать этот процесс, чтобы он реально заработал. Бэкстейдж-часть очень важна - она уже явно поясняет твоей технической команде, какие действия нужно выполнить. Данное разбиение на конкретные шаги может использоваться как для написания качественных PRD (Product Requirements Documents), так и для отдельных тикетов.

Конечно, реальные сервисы такси типа Uber и их Service Blueprint еще сложнее! Например, они позволяют сохранять популярные места, интегрироваться с работодателем (Uber Business), предлагают доставить еду, пока ты едешь! Хорошие новости в том, что построение более длинной таблички концептуально ничем не отличается от той, которую мы только что сделали.
...
По-хорошему, нам надо разобраться еще и со стороной водителя - там CJM немного другой, и архитектура без понимания этой части будет неполной. Я начала делать [Service Blueprint для водителя](https://miro.com/app/board/uXjVKachqeQ=/?moveToWidget=3458764584147340954&cot=14) сама.

Если ты уже чувствуешь себя уверенно, то можешь смело оставить это мне. Если хочешь еще попрактиковаться - попробуй доделать табличку до конца и заполни пропущенные события на бэкэнде и разнообразные саппорт-действия. Сюда загрузи скриншот.

Пожалуйста, не торопись, спокойно подумай над задачей хотя бы пару часов, а лучше - дней. Внимательно посмотри на шаги CJM, на каждую деталь. Напомни себе, что почти каждое действие в приложении требует работы сервиса и попытайся придумать хотя бы 2 стикера на шаг. Будет сложно, и это нормально - тебе надо набивать руку и другого пути, кроме как разбирать кейсы, просто нет. На пятом кейсе у тебя появится уверенность, а пока - работай.

![](https://app.productdo.it/media/tasks/en/tech_arch_blue_driver_driver_1_RNYBbr7.png)
...
Что скажешь по поводу моей таблички? Ее можно открыть в новом окне и увеличить.

![](https://app.productdo.it/media/tasks/ru/techpm_arch_blue_driver_check_techpm_arch_blue_driver_check_full_ru.png)

Молодец, мы еще попрактикуемся.

Итак, теперь ты видишь полную картину происходящего в приложении такси как с точки зрения пассажира, так и со стороны водителя.
...
Обрати внимание, что Service Blueprint - одномерная картинка, то есть она представляет **срезы действий пользователя** (зарегистрировался, вызвал такси, оплатил). При этом она не отвечает на вопрос, из каких же сервисов/баз данных/API состоит бэкэнд. Для этого нам нужна двухмерная схема - Service Diagram.

Вместе эти два инструмента отвечают на вопросы “как выглядит система вообще (SD) и как она реагирует на каждый пользовательский сценарий” (SBP).

Советую тебе немного отдохнуть и дать мозгу переварить новую концепцию. А уже завтра тебе ждет CTO, будете вместе с ним рисовать архитектуру. В трудные моменты обязательно заглядывай в Service Blueprint-ы пассажира и водителя!
